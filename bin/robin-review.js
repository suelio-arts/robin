...
const { DEFAULT_OWNER, isRobinWorkflow, robinWorkflowPin } = require("./robin-workflow-ref");
...
const robinWorkflows = workflowFiles.filter((candidate) =>
  isRobinWorkflow(fs.readFileSync(candidate, "utf8"))
);

// Preserve the owner as well as the ref: a repository pinned to the fork must
// not be silently moved back to the public action by re-running the installer.
let selectedRef = ref;
let selectedOwner = DEFAULT_OWNER;
if (!process.env.ROBIN_REF) {
  ...
  const canonicalSource = fs.existsSync(workflowPath)
    ...
    : robinWorkflows.length === 1
      ...
      : "";
  const pin = robinWorkflowPin(canonicalSource);
  if (pin && pin.ref !== "v0") {
    selectedRef = pin.ref;
    selectedOwner = pin.owner;
  }
}
...
jobs:
  review:
    uses: ${selectedOwner}/robin/.github/workflows/review.yml@${selectedRef}
    secrets:
...
if (isRobinSourceRepository) {
  ...
  if (fs.existsSync(workflowPath) && isRobinWorkflow(fs.readFileSync(workflowPath, "utf8"))) {
