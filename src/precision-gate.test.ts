...
describe("precision gate", () => {
  it("assigns stable IDs, removes exact duplicates, and keeps only approved IDs", () => {
    ...
    const selected = selectApprovedCandidates(candidates, '{"approved":{"c2":{"trigger":"input","path":"handler","impact":"failure","evidence":"a.ts:1"}},"rejected":{"c1":"not proven"}}', "Verified summary");
  ...
  it("rejects unusable gate responses and tolerates sparse candidates", () => {
    ...
    expect(() => selectApprovedCandidates(candidates, '{"approved":{"c999":{"trigger":"x","path":"x","impact":"x","evidence":"x"}},"rejected":{"c1":"no"}}')).toThrow(/every candidate/);
    expect(() => selectApprovedCandidates(candidates, '{"approved":{},"rejected":{}}')).toThrow(/every candidate/);
    expect(() => selectApprovedCandidates(candidates, '{"approved":{"c1":{"trigger":"","path":"x","impact":"x","evidence":"x"}},"rejected":{}}')).toThrow(/approved proof objects/);
    expect(() => selectApprovedCandidates(candidates, '{"approved":{"c1":{"trigger":"x","path":"x","impact":"x","evidence":"x"}},"rejected":{"c1":"no"}}')).toThrow(/every candidate/);
    expect(() => selectApprovedCandidates(candidates, '{"approved":{"c1":{"trigger":"x","path":"x","impact":"x","evidence":"x"}},"rejected":[]}')).toThrow(/rejected reason object/);
