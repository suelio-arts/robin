...
export const PRECISION_INSTRUCTIONS = [
  ...
  "A shared module-level cache lookup is different: when the changed function accepts both tenant identity and resource identity but keys the cache only by resource before validating ownership, two valid calls with different tenants and the same resource prove the cross-tenant collision directly. Do not require an external caller for that intrinsic state transition.",
