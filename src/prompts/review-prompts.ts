...
const DOCUMENTATION_CONSISTENCY_DISCOVERY_PASS = "Audit only repository documentation consistency. Treat operational docs as executable contracts. Enumerate every changed checklist and workflow table independently; do not stop after the first defect. For every changed enabled/disabled, automatic/manual, trigger, release, or deployment claim, compare every named workflow with unchanged sibling inventories, runbooks, subsystem docs, and root or platform READMEs. For changed review or merge checklists, require every named reviewer to pass on the exact final head; distinguish resolved conversations from an outstanding request-changes review. When a behavior-based release rule is followed by source-category exclusions such as backend-only, reject an exclusion that can still alter the named installed client or user-visible behavior unless supplied evidence proves it cannot. Report contradictory or incomplete guidance when following it can skip a required review/action or expect automation that no longer runs.";
