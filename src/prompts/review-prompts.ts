...
export const VERIFICATION_INSTRUCTIONS = [
  ...
  "Reconcile the candidates as one review before keeping any. When two candidates require incompatible contracts or fixes, use exact repository evidence to select the proven one; when evidence selects neither, keep only the candidate with the stronger changed-line anchor at reduced confidence and reject the other. Never publish contradictory findings.",
  "Treat repository-specific reviewer instructions as authoritative policy. Reject compatibility fallbacks, speculative abstractions, and migration advice that contradict an explicit hard-cut policy unless exact repository evidence proves a live migration requirement.",
  "For platform command flags, binary formats, and claims that supplied valid-looking syntax is illegal, model memory is not evidence: require exact supplied source, build or test output, correct byte-offset evidence, or authoritative documentation. Well-established core language evaluation order and standard-library semantics remain valid evidence.",
  ...
  "Do not reject a changed test, fixture, validator, pre-push gate, release check, or stress harness merely because it is test code. Keep it when it is the required enforcement boundary and the exact changed check can false-pass executable behavior, or when changed setup or timing makes the required gate fail. Reject mutation-coverage wish lists, comment/string lookalikes without a reachable bypass, and duplicate test findings when an implementation finding already captures the root cause.",
...
export const PRECISION_INSTRUCTIONS = [
  ...
  "Evaluate every candidate ID, then reconcile the approved set as one coherent review. Approve a candidate only when the changed line proves a reachable trigger, concrete failing path, and material impact. Never approve mutually incompatible findings or fixes; exact repository evidence should select the proven one, otherwise keep only the candidate with the stronger changed-line anchor at reduced confidence.",
  "Repository-specific reviewer instructions are authoritative policy. Reject a candidate or recommendation that adds a fallback, compatibility shim, speculative abstraction, or migration path forbidden by those instructions unless exact repository evidence proves it is required.",
  "For platform command flags, binary formats, and claims that supplied valid-looking syntax is illegal, model memory is not evidence: require exact supplied source, build or test output, correct byte-offset evidence, or authoritative documentation. Well-established core language evaluation order and standard-library semantics remain valid evidence.",
  ...
  "Reject mutation-test wish lists: a validator finding must prove that its changed contract claims a specific reachable state or boundary that it omits, not merely that a hypothetical future implementation change could pass. Do not demand exhaustive type, truthiness, numeric-boundary, comment, string, inert-markup, or alternate-spelling cases without a repository requirement tying that exact case to executable behavior. When an implementation candidate already captures the root cause, reject redundant test-hardening candidates.",
