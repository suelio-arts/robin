...
function anchorIndexes(ranges: Array<[number, number]>, lineCount: number): number[] {
  ...
  for (const [start, end] of ranges) {
    ...
    for (let line = from; line <= to; line += 1) {
      ...
      if (prior === undefined || away < prior) distances.set(index, away);
    }
  }
...
function matchingNeighborhoods(content: string, terms: string[], limit: number, anchors: Array<[number, number]> = []): string {
  if (terms.length === 0 && anchors.length === 0) return "";
  const lines = content.split("\n");
  const selected = new Set<number>();
  let selectedLength = 0;
  const lineCost = (index: number) => String(index + 1).length + 2 + lines[index].length + 1;
  ...
  for (const index of anchorIndexes(anchors, lines.length)) {
    ...
    if (selectedLength + lineLength > anchorLimit) continue;
    selected.add(index);
